  ...
  <div>
    <v-row>
      <v-col cols="12" xs="12" sm="12" md="12" lg="12" xl="12">
        <v-item-group>
          <v-card elevation="5" class="mx-auto transition-swing" width="600">
      ...
      try {
        ...
        for (let i = (lines.length-103); i < (lines.length-86); i++) {
          ...
          for (let j = 0; j < 1; j++) {
            if (
              (currentline[0] != "Total") &
              (currentline[0] != "Se desconoce región de origen")
            ) {
              this.data.push(
                new this.objetoDatos(currentline[0], currentline[3])
              );
            }
          }
        }
